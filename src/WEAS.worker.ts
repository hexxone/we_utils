...
class AudioProcessor {
    ...
    public runWithSettings(data, settings, lastData?) {

        // fix pink noise?
        if (settings.equalize) this.correctPinkNoise(data);

        // write botch channels to mono
        if (settings.mono_audio) this.stereoToMono(data);

        // normal high & low mapping
        if (settings.audio_direction == 0) {
            // only flip the second half of the data
            if (!settings.mono_audio) this.invertSecond(data);
        }
        // flipped high & low mapping
        else {
            // flip whole range
            if (settings.mono_audio) this.invertAll(data);
            // only flip first half of stereo
            else this.invertFirst(data);
        }

        // process peaks?
        if (settings.peak_filter > 0) this.peakFilter(data, settings.peak_filter + 1);
    ...
    private applyValueLeveling(curr, prev, sett) {
...
ctx.addEventListener("message", (e) => {
    ...
    proc.runWithSettings(data, settings, lastData);
