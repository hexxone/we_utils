...
const pNoise = [0.97609734030594, 0.85207379418243, 0.68842437227852, 0.63767902570829,
    ...
    0.84797007577483, 0.8573583910469, 0.96382997811663, 1.09819377577185, 1.1628692615814,
    1.2059083969751, 1.2819808497335, 1.357092297208, 1.3726521464753, 1.3735992532905,
...
function equalizePeaks(array: Float64Array): void {
    ...
    for (i = 0; i < DAT_LEN; i++) {
		...
		if(array[i] >= peakData[i]) {
			...
			minStrength[i] = minStrength[i] / 2;
		} 
		else {
            // peak decrementing if too weak
			if(peakStrength[i] < 1) peakData[i] *= 0.97; 
			else peakStrength[i] -= (peakData[i] - array[i]);
		}
		// min edging
		if(array[i] <= minData[i]) {
            ...
            // min incrementing if too wek
			if(minStrength[i] < 1) minData[i] *= 1.01; 
			else minStrength[i] -= (array[i] - minData[i]);
		}
		...
		array[i] = array[i] * (1.0 - tmpF) + (emult < 0 ? 0 : (emult > 1.2 ? 1.2 : emult)) * tmpF;

		// new max
        if (array[i] > newMax) newMax = array[i];
	}
...
function peakFilter(array: Float64Array, amount: f64): void {
    ...
    for (i = 0; i < DAT_LEN; i++) {
        if (array[i] > oldMax) oldMax = array[i];
        tempArr[i] = Math.pow(array[i] * amount, amount) as f64;
        if (tempArr[i] > newMax) newMax = tempArr[i];
    }
    // re-scale & apply
    tmpF = newMax / oldMax;
    for (i = 0; i < DAT_LEN; i++)
...
export function update(): void {
	...
	if(audioSettings[5] < 100 || audioSettings[6] < 100)
		applyValueLeveling(workData, outputData,
				audioSettings[5],
				audioSettings[6]);
    ...
    for (indx = 0; indx < DAT_LEN; indx++) {
        ...
        if (tmpF > max) max = tmpF;
        // process ranges
        if (indx < 16) bass += tmpF * audioSettings[9];
        else if (indx > 69) peaks += tmpF * audioSettings[7]; 
        else mids += tmpF * audioSettings[8]; 
        // calc peak average
        sum += tmpF;
    }
    ...
    for (indx = 0; indx < DAT_LEN; indx++) {
		if(workData[indx] > average) intensity += (workData[indx] - average) * 0.5;
	}
